...
;	-----------------------------------------
_output_leds:
;	main.c: 79: __asm__("    mov a, _bytes              ;move our byte count into a");
	mov	a, _bytes              ;move our byte count into a
;	main.c: 80: __asm__("    mov _byte_ctr, a           ;move a into our index counter");
	mov	_byte_ctr, a           ;move a into our index counter
;	main.c: 81: __asm__("    mov	a, #(_pixel_buff)   ;load the address of the array into a");
	mov	a, #(_pixel_buff)   ;load the address of the array into a
;	main.c: 82: __asm__("    mov	p, a                ;put that address into p (a mem location)");
	mov	p, a                ;put that address into p (a mem location)
;	main.c: 83: __asm__("00010$:");
	00010$:
;	main.c: 84: __asm__("    idxm	a, p                ;load the byte data from the array into a");
	idxm	a, p                ;load the byte data from the array into a
;	main.c: 85: __asm__("    inc p                      ;inc p to the next address");
	inc	p                      ;inc p to the next address
;	main.c: 86: __asm__("    mov	_byte_t+0, a        ;load the byte data into byte_t");
	mov	_byte_t+0, a        ;load the byte data into byte_t
;	main.c: 87: __asm__("    mov a, #0x08               ;reset the bit count");
	mov	a, #0x08               ;reset the bit count
;	main.c: 88: __asm__("00011$:");
	00011$:
;	main.c: 89: __asm__("    ;output bit");
;output	bit
;	main.c: 90: __asm__("    set1.io __pa, #5           ;set the output pin high");
	set1.io	__pa, #5           ;set the output pin high
;	main.c: 91: __asm__("    t1sn _byte_t, #7           ;if the current MSB is 1, skip setting the pin low now");
	t1sn	_byte_t, #7           ;if the current MSB is 1, skip setting the pin low now
;	main.c: 92: __asm__("    set0.io __pa, #5           ;set the output pin low");
	set0.io	__pa, #5           ;set the output pin low
;	main.c: 93: __asm__("    nop                        ;wait one cycle");
	nop	                       ;wait one cycle
;	main.c: 94: __asm__("    set0.io __pa, #5           ;set the output pin low");
	set0.io	__pa, #5           ;set the output pin low
;	main.c: 95: __asm__("    sl _byte_t                 ;left-shift the data byte (the LEDs are MSB first)");
	sl	_byte_t                 ;left-shift the data byte (the LEDs are MSB first)
;	main.c: 96: __asm__("    dzsn a                     ;dec a and skip next instruction if 0");
	dzsn	a                     ;dec a and skip next instruction if 0
;	main.c: 97: __asm__("    goto 00011$                ;jump back to 00011");
	goto	00011$                ;jump back to 00011
;	main.c: 98: __asm__("    dzsn _byte_ctr             ;dec our index counter and skip the goto if 0");
	dzsn	_byte_ctr             ;dec our index counter and skip the goto if 0
;	main.c: 99: __asm__("    goto	00010$              ;goto the next byte");
	goto	00010$              ;goto the next byte
;	main.c: 101: }
	ret
;	main.c: 103: void update_pattern()
;	-----------------------------------------
;	 function update_pattern
;	-----------------------------------------
_update_pattern:
;	main.c: 108: switch (pattern_index)
...
;	main.c: 110: case 0:
00101$:
;	main.c: 111: copy_to_buff(off_color, LED_COUNT);
	mov	a, #(_off_color + 0)
...
;	main.c: 113: case 1:
00102$:
;	main.c: 114: copy_to_buff(blue_color, LED_COUNT);
	mov	a, #(_blue_color + 0)
	mov	___memcpy_PARM_2+0, a
	clear	___memcpy_PARM_2+1
	mov	a, #(_copy_to_buff_PARM_1 + 0)
	...
	mov	_copy_to_buff_PARM_2+0, a
	call	_copy_to_buff
;	main.c: 115: bright = bright_blue_color;
...
;	main.c: 117: case 2:
00103$:
;	main.c: 118: copy_to_buff(orange_color, LED_COUNT);
	...
	mov	_copy_to_buff_PARM_2+0, a
	call	_copy_to_buff
;	main.c: 119: bright = bright_orange_color;
	mov	a, #(_update_pattern_bright_10000_19 + 0)
	mov	___memcpy_PARM_1+0, a
	clear	___memcpy_PARM_1+1
...
;	main.c: 121: }
00104$:
;	main.c: 123: pattern_step++;
	mov	a, _pattern_step+0
	add	a, #0x01
	mov	_pattern_step+0, a
;	main.c: 125: switch (pattern_step)
	mov	a, _pattern_step+0
	cneqsn	a, #0x01
	goto	00105$
00175$:
...
;	main.c: 127: case 1:
00105$:
;	main.c: 128: pixel_buff[0] = bright;
	mov	a, #(_pixel_buff + 0)
	mov	___memcpy_PARM_1+0, a
	clear	___memcpy_PARM_1+1
	mov	a, #(_update_pattern_bright_10000_19 + 0)
...
;	main.c: 130: case 3:
00106$:
;	main.c: 131: pixel_buff[1] = bright;
	mov	a, #(_pixel_buff + 3)
	mov	___memcpy_PARM_1+0, a
	clear	___memcpy_PARM_1+1
	mov	a, #(_update_pattern_bright_10000_19 + 0)
...
;	main.c: 133: case 4:
00107$:
;	main.c: 134: pixel_buff[2] = bright;
	mov	a, #(_pixel_buff + 6)
	mov	___memcpy_PARM_1+0, a
	clear	___memcpy_PARM_1+1
	mov	a, #(_update_pattern_bright_10000_19 + 0)
...
;	main.c: 137: case 7:
00109$:
;	main.c: 138: pixel_buff[3] = bright;
	mov	a, #(_pixel_buff + 9)
	mov	___memcpy_PARM_1+0, a
	clear	___memcpy_PARM_1+1
	mov	a, #(_update_pattern_bright_10000_19 + 0)
...
;	-----------------------------------------
_main:
;	main.c: 149: __disgint();
	disgint
;	main.c: 150: CLKMD |= 0x10; // First enable IHRC
	set1.io	__clkmd, #4
;	main.c: 151: CLKMD = 0x34;  // Switch to IHRC/2 but leave ILRC on
	mov	a, #0x34
	mov.io	__clkmd, a
;	main.c: 152: CLKMD &= ~(CLKMD_ENABLE_WATCHDOG);
	set0.io	__clkmd, #1
;	main.c: 153: PAC = LED_bm;     // Pin 5 as out
	mov	a, #0x20
	mov.io	__pac, a
;	main.c: 154: PAPL = BTN_bm;    // Pull Pin 4 low internally
	mov	a, #0x10
	mov.io	__papl, a
;	main.c: 155: PAC &= ~(BTN_bm); // Pin 4 as in
	set0.io	__pac, #4
;	main.c: 156: PADIER |= BTN_bm; // Enable digital in
	set1.io	__padier, #4
;	main.c: 159: INTEN = INTEN_TM3;
	mov	a, #0x80
	mov.io	__inten, a
;	main.c: 162: TM3B = 10;
	mov	a, #0x0a
	mov.io	__tm3b, a
;	main.c: 163: TM3C = TM3C_CLK_ILRC | TM3C_OUT_DISABLE | TM3C_MODE_PERIOD;
	mov	a, #0x40
	mov.io	__tm3c, a
;	main.c: 164: TM3S = TM3S_PRESCALE_DIV64; // should also start the timer
	mov	a, #0x60
	mov.io	__tm3s, a
;	main.c: 166: TM2B = 10;
	mov	a, #0x0a
	mov.io	__tm2b, a
;	main.c: 167: TM2C = TM2C_CLK_DISABLE | TM2C_OUT_PA3 | TM3C_MODE_PERIOD;
	mov	a, #0x08
	mov.io	__tm2c, a
;	main.c: 168: TM2S = TM2S_PRESCALE_DIV16;
	mov	a, #0x40
	mov.io	__tm2s, a
;	main.c: 170: __engint();
	engint
;	main.c: 172: start_tone_out_startup();
...
;	main.c: 176: }
	ret
;	main.c: 178: void interrupt(void) __interrupt(0)
;	-----------------------------------------
;	 function interrupt
;	-----------------------------------------
_interrupt:
	...
	mov	a, p
	push	af
;	main.c: 181: if (INTRQ & INTRQ_TM3) // Timer3 interrupt request
	mov.io	a, __intrq
	and	a, #0x80
	cneqsn	a, #0x00
	goto	00116$
00160$:
;	main.c: 184: if (PA & BTN_bm)
	mov.io	a, __pa
	and	a, #0x10
	cneqsn	a, #0x00
	goto	00106$
00161$:
;	main.c: 187: if (!button_down)
	mov	a, _button_down+0
	ceqsn	a, #0x00
	goto	00107$
00162$:
;	main.c: 189: button_ctr++;
	mov	a, _button_ctr+0
	add	a, #0x01
	mov	_button_ctr+0, a
;	main.c: 191: if (button_ctr == BTN_DEBOUNCE_DELAY)
	mov	a, _button_ctr+0
	ceqsn	a, #0x05
	goto	00107$
00163$:
;	main.c: 193: button_down = 1;
	mov	a, #0x01
	mov	_button_down+0, a
;	main.c: 194: pattern_index = (pattern_index + 1) % 3;
	mov	a, _pattern_index+0
	add	a, #0x01
...
;	main.c: 195: pattern_step = 0;
	clear	_pattern_step+0
;	main.c: 196: start_tone_out_button();
	call	_start_tone_out_button
	goto	00107$
00106$:
;	main.c: 202: button_ctr = 0;
	clear	_button_ctr+0
;	main.c: 203: button_down = 0;
	clear	_button_down+0
00107$:
;	main.c: 206: pattern_count++;
	mov	a, _pattern_count+0
	add	a, #0x01
	mov	_pattern_count+0, a
;	main.c: 208: if (pattern_count == PATTERN_UPDATE_DELAY)
	mov	a, _pattern_count+0
	ceqsn	a, #0x32
	goto	00109$
00164$:
;	main.c: 210: update_pattern();
	call	_update_pattern
;	main.c: 211: output_leds();
	call	_output_leds
;	main.c: 212: pattern_count = 0;
	clear	_pattern_count+0
00109$:
;	main.c: 215: if (tone_time)
	mov	a, _tone_time+0
	cneqsn	a, #0x00
	goto	00113$
00165$:
;	main.c: 217: tone_out_ctr++;
	mov	a, _tone_out_ctr+0
	add	a, #0x01
	mov	_tone_out_ctr+0, a
;	main.c: 219: if (tone_out_ctr >= tone_time)
	mov	a, _tone_out_ctr+0
	sub	a, _tone_time+0
;	main.c: 221: stop_tone_out();
	t1sn.io	f, c
	call	_stop_tone_out
00113$:
;	main.c: 225: INTRQ &= ~INTRQ_TM3; // Clear interrupt flag
	set0.io	__intrq, #7
00116$:
;	main.c: 227: }
	pop	af
